...
class Spillbrett:
    ...
    def legg_til_objekter_fra_fil(self, filnavn):
        ...
        apen_fil = open(str(filnavn))
        # håndterer hver linje for seg
        for linje in apen_fil:
            biter = linje.strip().split()
            #deler hver linje inn i hva vi skal opprette og hvilke koordinater dette har
            objekt = str(biter[0])
            plassering_x = int(biter[1])
            plassering_y = int(biter[2])
            
            #trenger ikke å gi bilder, da jeg fjernet disse parameterne
            if objekt == "ulv":
                self.opprett_ulv(plassering_x, plassering_y)
            elif objekt == "sau":
                self.opprett_sau(plassering_x, plassering_y)
            elif objekt == "gress":
                self.opprett_gress(plassering_x, plassering_y)
            elif objekt == "stein":
                self.opprett_stein(plassering_x, plassering_y)
    ...
    def opprett_sau(self, x, y):
        #trenger ikke å gi bilder, da jeg fjernet disse parameterne
        self._sauer.append(Sau(x, y, self))

    def opprett_gress(self, x, y):
        #trenger ikke å gi bilder, da jeg fjernet disse parameterne
        self._gress.append(Gress(x, y))

    def opprett_ulv(self, x, y):
        #trenger ikke å gi bilder, da jeg fjernet disse parameterne
        self._ulv = Ulv(x, y, self)

    def opprett_stein(self, x, y):
        #trenger ikke å gi bilder, da jeg fjernet disse parameterne
        self._steiner.append(Stein(x, y))

    def skaler_fart(self, objekt, maksfart):
        fart = math.sqrt(objekt._fart_fra_topp**2 +
                         objekt._fart_fra_venstre**2)
        if fart == 0:
            return
    ...
    def start_bevegelse(self, objekt):
        ...
        if tid_brukt > 0.2:
            print(
                "ADVARSEL! Et objekt har brukt lang tid (%.4f sek) på å tenke" % tid_brukt)

        beveg_y = 0
        beveg_x = 0
        if objekt.retning() == Retning.NED:
            beveg_y = 1
        elif objekt.retning() == Retning.HOYRE:
            beveg_x = 1
        elif objekt.retning() == Retning.VENSTRE:
            beveg_x = -1
        elif objekt.retning() == Retning.OPP:
            beveg_y = -1
        else:
            return
    ...
    def oppdater(self):
        ...
        if self._oppdatering == self._antall_runder:
            print(" ========= Spillet er over, %d runder er utført ==========" %
                  self._antall_runder)
            print("Sauer:")
            for sau in self._sauer:
                if not sau.er_spist():
    ...
    # lagde disse for å kunne ta de inn senere som lister 
    # kan finne nærmeste gresset
    # kan også sjekke gjennom listen over steiner om de finnes på neste bevegelse
    def hent_gress(self):
        return self._gress
